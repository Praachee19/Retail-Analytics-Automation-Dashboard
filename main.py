...
# Add file uploader
uploaded_file = st.file_uploader("Upload the latest low stock alert CSV file", type=["csv"])
...
if uploaded_file is not None:
    ...
    # Assuming retail_dataset.csv is in the same directory as the alerts file if needed
    # For simplicity, we won't handle uploading the retail dataset file in this modification.
    # You can add another file uploader if needed for data.
    # data_path = os.path.join(os.path.dirname(uploaded_file.name), "retail_dataset.csv")
...
if alerts is not None:
    ...
    regions = ["All"] + sorted(alerts["region"].dropna().unique().tolist()) if "region" in alerts.columns else ["All"]
    states = ["All"] + sorted(alerts["state"].dropna().unique().tolist()) if "state" in alerts.columns else ["All"]
    cities = ["All"] + sorted(alerts["city"].dropna().unique().tolist()) if "city" in alerts.columns else ["All"]
    cats = ["All"] + sorted(alerts["category"].dropna().unique().tolist()) if "category" in alerts.columns else ["All"]
    ...
    with colA:
        if "state" in f_alerts.columns and not f_alerts.empty:
        ...
        elif "state" not in f_alerts.columns:
            st.warning("State column not found in the uploaded file.")
        else:
            st.info("No data to display for low stock count by state.")

    with colB:
        if "store_code" in f_alerts.columns and not f_alerts.empty:
        ...
        elif "store_code" not in f_alerts.columns:
            st.warning("Store Code column not found in the uploaded file.")
        else:
            st.info("No data to display for top stores by low stock count.")
    ...
    st.warning("Please upload a CSV file to see the alerts.")
    st.stop()